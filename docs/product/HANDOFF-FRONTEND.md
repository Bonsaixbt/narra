...
| GET | `/history/cluster/:slug?hours=24&window=60m` | `{ slug, id, window, hours, snapshots: [{ts, ts_unix, window, status, id, n_launches, quote_eth, buyers, graduations, members}], reading }` — one window per answer (60m unless asked); `:slug` may be a meta id (`slug@first_seen_ts`) to follow the meta through slug changes | holders |
