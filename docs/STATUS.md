...
| Обработка реоргов глубже 2 блоков: перечитывать хвост, сверять block_hash | не сверяет хэши; на Arbitrum-стеке редкость | `src/ingest/sync.ts` |
...
| Точная ротация «за последние 10 минут» в вердикте | сейчас пересечение считается по всему окну | `src/analyze/verdict.ts` |
