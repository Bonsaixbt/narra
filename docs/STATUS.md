...
| История: `narra backfill --hours N` на архивном RPC, параллельные чанки, почасовые агрегаты после ретеншна, окна 24h/7d | движение и накопление капитала видно только на днях, не на часах | `src/ingest/sync.ts`, `src/store/db.ts` |
| Трейдеры: `wallet_stats` (PnL в ETH по кривым и пулам, время входа после запуска, доля покупок под tax, число мет), когорты sniper / sprayer / rotator / early-in-hot, `narra wallets`, `narra wallet <addr>`; в тепле `rotator_share`, в вердикте причина «N ротаторов вошли за 10 мин» | поток капитала между метами по когортам, без copytrade | план `docs/superpowers/plans/2026-09-13-narra-v0.2.md`, задача 3 |
