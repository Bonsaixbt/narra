...
const frame = (lines: { text: string; size?: number; color?: string; weight?: number }[]) => `<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630"><rect width="1200" height="630" fill="#0B0C0E"/><g font-family="DejaVu Sans Mono, Menlo, monospace" fill="#E6E7EA">${lines.map((l, i) => `<text x="64" y="${120 + i * 66}" font-size="${l.size ?? 34}" font-weight="${l.weight ?? 400}" fill="${l.color ?? "#E6E7EA"}">${esc(l.text)}</text>`).join("")}</g><text x="64" y="590" font-family="DejaVu Sans Mono, Menlo, monospace" font-size="22" fill="#8A8F99">narra · IN means membership in a live meta, not a recommendation · ${esc(new Date().toISOString().slice(11, 16))} UTC</text></svg>`;
