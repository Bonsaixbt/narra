...
app.get("/api/wallet-clusters", gated, async (c) => { const rw = resolveWindow(c); if (rw instanceof Response) return rw; const { w, cached } = rw; return c.json(await engine.n.walletClusters({ analysis: cached, window: w, top: Number(c.req.query("top") ?? 5000) })); });
