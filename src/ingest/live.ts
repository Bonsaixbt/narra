 ...
 * says "something happened, sync now". A log on the factory (launch, sweep, graduation) wakes the loop, debounced to
 * one tick per `minGapMs`. The PoolManager is deliberately not subscribed: every swap, liquidity change and donate on
 * it is one WebSocket message, hundreds of thousands a day against the RPC quota, and the polling tick catches swaps
 * within its interval anyway. A watchdog re-subscribes after 45 s of silence and the caller's polling
...
export interface LiveHandle { stop(): void; health(): { mode: "websocket" | "polling"; lastEventAt: number; recoveries: number; note: string; events: number; wakes: number } }
...
export function liveTrigger(ws: PublicClient | null, onWake: (reason: string) => void, opts: { minGapMs?: number; quietMs?: number } = {}): LiveHandle {
  ...
  const h = { mode: (ws ? "websocket" : "polling") as "websocket" | "polling", lastEventAt: Date.now(), recoveries: 0, note: ws ? "" : "no websocket configured", events: 0, wakes: 0 };
  ...
  const wake = (reason: string) => {
    ...
    pending = setTimeout(() => { pending = null; lastWake = Date.now(); h.wakes++; onWake(reason); }, wait);
  };
  const subscribe = () => {
    for (const u of unwatch) u();
    unwatch = [
      ws.watchEvent({ address: ADDR.ponsFactory, onLogs: (logs) => { h.events += logs.length; wake("launch"); }, onError: (e) => { h.note = `factory sub: ${e.message.split("\n")[0].slice(0, 80)}`; } }),
    ];
  };
