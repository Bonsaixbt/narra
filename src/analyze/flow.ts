...
export function flowEdges(membership: Map<string, string>, trades: TradeRow[], launches: LaunchRow[], window: Window, opts: FlowOptions = DEFAULT_FLOW_OPTIONS): Edge[] {
  ...
  const edge = (a: string, b: string) => { const k = `${a}→${b}`; let e = edges.get(k); if (!e) { e = { from: a, to: b, wallets: 0, quote_norm: 0, deployers: 0 }; edges.set(k, e); } return e; };
  for (const [w, prev] of prevBuys) {
    ...
    for (const [a, toks] of prev) {
      ...
      for (const [b, q] of curr) { if (a === b) continue; const e = edge(a, b); e.wallets++; e.quote_norm += q; }
    }
  }
  // deployers
  ...
  return [...edges.values()]
    ...
    .map((e) => ({ ...e, quote_norm: Math.round(e.quote_norm * 1000) / 1000 }))
