...
import { readTrend } from "../analyze/readings.js";
...
export async function trend(args: Args): Promise<number> {
  ...
  try {
    const t = computeTrend(n.store, hours, step);
    const { since, narratives: topNar, rows: out } = t;
    const reading = readTrend(t);
    if (args.flags.json) { printJson({ hours, step, since, narratives: topNar, rows: out, reading }); return 0; }
